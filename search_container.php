...
<body>
   ...
   <?php include 'images_store_config.php';
   ...
   //number of pages I want in every page 
                ...
                $total = "SELECT * FROM images_store WHERE image_name LIKE '%$name%'";
                ...
                $sql = "SELECT * FROM images_store WHERE image_name LIKE '%$name%' LIMIT $offset, $items_per_page";
                $execute = mysqli_query($connection, $sql);
                ?>

        <h2>Results based on: <?php echo $name; ?></h2>
        <div class="search_gallery" id="search_gallery">
<?php
if($number_of_rows > 0){
    while($row_details = mysqli_fetch_assoc($execute)){?>
                    <figure><img src="<?php echo $row_details['image_path']; ?>" alt="<?php echo $row_details['image_name']; ?>" class='loading'><section><?php echo $row_details['image_name']; ?></section>
                    </figure>
    <?php } 
}else{
        $message = "No result matched your search '$name'. Try another keyword...";
}?>
            </div>
             <?php if(isset($message)){ echo "<h6 style = 'padding-bottom: 70%; margin-left: 5px;width: 80%; font-size: 3vmin; font-family: Georgia;'>$message</h6>"; } ?>
        </div>
    ...
    <?php
        if($page > 1){
            echo '<a href="search_container.php?page='.($page - 1).'"><button class="paging_previous">Previous</button></a>';
        }
        ?>
        <div class="pages">
            <?php
            for($i = 1; $i <= $number_of_pages; $i++){
                if($number_of_pages > 1){
                    if($i == $page){
                        ...
                        echo '<a href="search_container.php?page='.$i.'">'.$i.'</a>';
                    }
                }
            }
            ?>
        ...
        if($number_of_pages > $page){
            echo '<a href="search_container.php?page='.($page + 1).'"><button class="paging_next">Next</button></a>';
        }
        ?>
    </div>

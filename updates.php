    ...
    <link href="https://fonts.googleapis.com/css2?family=Exo+2:ital@1&display=swap" rel="stylesheet">
        ...
        ?>
           ...
           <nav class = "buttons">
              ...
              <div id="bar"><a href="">Updates</a></div>
        ...
        <div class="menu">
            ...
            <div id="dropdown">
               ...
               <div id="dropdown_selected"><a href=""><i class="far fa-bell"></i>Updates</a></div>
...
<body>
    ...
    <div class = "content">
        ...
        <div class="Gallery_1" id="Gallery_1">
            ...
            $total = "SELECT * FROM images_store WHERE (DATEDIFF(CURDATE(), upload_date)) <= 30 ORDER BY upload_date";
            $result = mysqli_query($connection, $total);
            $number_of_rows = mysqli_num_rows($result);
            if($number_of_rows == 0){
                $notice = 'Ooops! No recently added cars';
            }
            //number of pages
            ...
            $sql = "SELECT * FROM images_store WHERE (DATEDIFF(CURDATE(), upload_date)) <= 30 ORDER BY upload_date LIMIT $offset, $items_per_page";
            $execute = mysqli_query($connection, $sql);

            while($row = mysqli_fetch_assoc($execute)){?>
                <figure><img src="<?php echo $row['image_path']; ?>" alt="<?php echo $row['image_name']; ?>" class='loading'><section><?php echo $row['image_name']; ?></section>
                </figure>
            <?php }?>
            </div>
            <?php if(isset($notice)){?><div id = 'update_notice'><?php echo $notice;?></div> <?php }?>
        </div>
    ...
    <?php
        if($page > 1){
            echo '<a href="updates.php?page='.($page - 1).'"><button class="paging_previous">Previous</button></a>';
        }
        ?>
        <div class="pages">
            <?php
            for($i = 1; $i <= $number_of_pages; $i++){
               if($i == $page){
                   ...
                   echo '<a href="updates.php?page='.$i.'">'.$i.'</a>';
               }
            }
            ?>
        </div>
        ...
        if($number_of_pages > $page){
            echo '<a href="updates.php?page='.($page + 1).'"><button class="paging_next">Next</button></a>';
        }
        ?>
    </div>
